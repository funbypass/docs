...
No SDK required — use the `requests` library:
...
class FunBypass:
    ...
    def get_task_result(self, task_id: str, interval: float = 0.5, timeout: int = 300) -> str:
        """Poll for task result. Returns the result token."""
        elapsed = 0
    ...
    def run_task(self, **kwargs) -> str:
        """Create a task and wait for the result. Returns the result token."""
...
class AsyncFunBypass:
    ...
    async def run_task(self, session: aiohttp.ClientSession, **task_params) -> str:
        # Create task
        async with session.post(f"{self.BASE_URL}/createTask", json={
            "clientKey": self.client_key,
            "task": {"type": "FInteractionsTask", **task_params},
        }) as resp:
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        result_token = await client.run_task(
            session,
